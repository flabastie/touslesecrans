...
class produitInsert
{
    ...
    private $ID_resolution;
      ...
      public function recup_idLabel($connexion, $_nomTable, $_nomLabel) {
        ...
        switch ($_nomTable) {
          case 'Marque':
            $stmt = $connexion->prepare("SELECT ID AS result FROM `Marque` WHERE Marque.label = ? ");
            break;

          case 'Technologie':
            $stmt = $connexion->prepare("SELECT ID AS result FROM `Technologie` WHERE label = ? ");
            break;

          case 'Taille':
            $stmt = $connexion->prepare("SELECT ID AS result FROM `Taille` WHERE label = ? ");
            break;

          case 'Resolution':
            $stmt = $connexion->prepare("SELECT ID AS result FROM `Resolution` WHERE label = ? ");
            break;

          case 'Luminosite':
            $stmt = $connexion->prepare("SELECT ID AS result FROM `Luminosite` WHERE label = ? ");
            break;
      ...
      public function insertInfosProduit(   $connexion,
                                            $_modele_02, 
                                            $_prix_03, 
                                            $_titre_04, 
                                            $_ref_fabriquant_05, 
                                            ...
                                            $_ID_resolution   ) 
      {
              ...
              $requete = 'INSERT INTO Produit (   `modele`,
                                                  `prix`,
                                                  `titre`,
                                                  `ref_fabriquant`,
                                                  `ref_vendeur`,
              ...
              // requete de récupération de l'ID nouvellement créée

              $stmt2 = $connexion->prepare(" SELECT MAX(ID) AS id_max FROM Produit");
              $stmt2->execute();
      ...
      public function insertInfosConnectique($connexion, $_tabConnectiques, $_ID_nouveauProduit)
      {
          $this->tabConnectiques = $_tabConnectiques;
          $this->ID_nouveauProduit = $_ID_nouveauProduit;
          ...
          foreach ($this->tabConnectiques as $valeur) 
          {
                ...
                $this->tab_ConnectiqueProduit[$id] = $this->ID_nouveauProduit;
          }
          ...
          $stmt2 = $connexion->prepare ("INSERT INTO Liaison ( `Connectique_ID`, `Produit_ID`) VALUES ( ? , ? )");
